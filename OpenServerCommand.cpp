...
unsigned OpenServerCommand::execute(vector<string>::iterator it_vec, unordered_map<string, Var> &var_map) {
    ...
    map<string, float> sim_Vars = initXml();
//  thread t1(&OpenServerCommand::receiveData, var_map, sim_Vars, client_socket);
...
map<string, float> OpenServerCommand::initXml() {
    string line;
...
void OpenServerCommand::receiveData(unordered_map<string, Var> &var_map,
                                    map<string, float> sim_Vars,
                                    int client_socket) {
//  while (!shouldStop) {
//    //todo lock
